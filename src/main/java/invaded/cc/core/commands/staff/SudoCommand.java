...
public class SudoCommand extends BasicCommand {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if(args[0].equals("all")) {
            ...
            switch (sudoType) {
                case CHAT:
                    Bukkit.getOnlinePlayers().forEach(target -> target.chat(message.toString()));
                    break;
                case COMMAND:
                    Bukkit.getOnlinePlayers().forEach(target -> target.performCommand(message.toString().replace("/", "")));
                    break;
            }
            return;
        }
