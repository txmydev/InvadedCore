...
import invaded.cc.core.tablist.client.ClientVersionUtil;
import invaded.cc.core.tablist.reflect.Reflection;
import invaded.cc.core.tablist.skin.SkinType;
...
public class TabAdapter_v1_8_R3 extends TabAdapter {

	private final Map<Integer, GameProfile> profiles = new HashMap<Integer, GameProfile>();
	private final List<Player> initialized = new ArrayList<Player>();
	
	@Override
	public TabAdapter sendEntryData(Player player, int column, int row, String text, int ping) {
		if (column > 4) {
		...
		packet.name = teamName;
		packet.displayName = teamName;

		packet.prefix = split[0];
		packet.suffix = split[1];
	...
	@Override
	public TabAdapter sendHeaderFooter(Player player, List<String> header, List<String> footer) {
	...
	@Override
	public TabAdapter setupScoreboard(Player player) {
		PacketPlayOutScoreboardTeam packet = new PacketPlayOutScoreboardTeam();
		packet.name = "tabHandler";
		packet.displayName = "tabHandler";

		packet.action = 0;
		packet.chatFormat = -1;

		packet.playerNames = Lists
		...
		for(int row = 0; row < 20; row++) {
			for(int column = 0; column < 4; column++) {
				...
				packet.action = 0;
				packet.chatFormat = -1;
				packet.playerNames = Collections.singleton(name);
		...
		packet.action = 3;
		packet.chatFormat = -1;
		packet.playerNames = Collections.singleton(player.getName());
