package invaded.cc.core.tablist.entry;
...
public class TabElement {
    ...
    private final List<invaded.cc.core.tablist.entry.TabEntry> entries = new ArrayList<>();
     ...
     * @return the entry
     */
    public invaded.cc.core.tablist.entry.TabEntry getEntry(int column, int row) {
        return this.entries.stream()
                .filter(entry -> entry.getColumn() == column && entry.getRow() == row)
                .findFirst().orElseGet(() -> new invaded.cc.core.tablist.entry.TabEntry(column, row, "", -1));
    }
    ...
    public void add(int column, int row, String text, int ping) {
        this.entries.add(new invaded.cc.core.tablist.entry.TabEntry(column, row, text, ping));
    }
