...
public final class LCWaypoint {
    ...
    public LCWaypoint(String name, Location location, int color, boolean forced, boolean visible) {
        this(name, location.getBlockX(), location.getBlockY(), location.getBlockZ(), Spotify.getInstance().getLunarHandler().getWorldIdentifier(location.getWorld()), color, forced, visible);
    }
