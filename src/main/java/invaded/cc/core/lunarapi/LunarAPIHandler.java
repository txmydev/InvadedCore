...
import invaded.cc.core.lunarapi.event.LCPacketReceivedEvent;
import invaded.cc.core.lunarapi.event.LCPacketSentEvent;
import invaded.cc.core.lunarapi.event.LCPlayerUnregisterEvent;
import invaded.cc.core.lunarapi.listener.LunarClientLoginListener;
import invaded.cc.core.lunarapi.nethandler.LCPacket;
import invaded.cc.core.lunarapi.nethandler.client.*;
...
import invaded.cc.core.lunarapi.object.LCWaypoint;
import invaded.cc.core.lunarapi.object.StaffModule;
...
public class LunarAPIHandler {

    private final Spotify plugin;

    @Getter
    private final List<String> channels = Arrays.asList("lunarclient:pm", "Lunar-Client");

    public LunarAPIHandler(Spotify plugin){
         this.plugin = plugin;
    ...
    public void onEnable() {
        ...
        channels.forEach(channel -> registerPluginChannel(channel));
        getServer().getPluginManager().registerEvents(new LunarClientLoginListener(plugin, this), plugin);
    }
    ...
    private void registerPluginChannel(String bukkitChannel) {
        ...
        messenger.registerIncomingPluginChannel(plugin, bukkitChannel, (channel, player, bytes) -> {
    ...
    public boolean sendPacket(Player player, LCPacket packet) {
        ...
        if (isRunningLunarClient(playerId)) {
            channels.forEach(channel -> {
                player.sendPluginMessage(plugin, channel, LCPacket.getPacketData(packet));
